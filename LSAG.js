...
contract('LSAG', async function (accounts){
    // Converts Big Number to hex strings
    const bn2hex = x => '0x' + x.toString(16).padStart(64, '0')
    let lsag;
    before(async () => {
        lsag = await LSAG.deployed();
    })

    describe("success states", async () => {
        /*
        it('test ring signature', async () => {
            const signing_key = '0x1cb0e68ec58bfa7863289b95c6d8eb9d9e66cf9f4804d5ebd346338ebad7fa6e';
            const key_idx = 1;
            const M = '0x45544820666f7220796f7520616e642065766572796f6e6521';
            const y = [
            ...
            var result =  rsig.ring_signature_hex(signing_key, key_idx, M, y);
            console.log(result);
        })
        ...
        it('intToPoint', async () => {
            ...
            const point = await lsag.intToPoint.call(secretKey)
        ...
        */
        ...
        it('verify', async () => {
            ...
            var result =  rsig.ring_signature_hex(signing_key, key_idx, message, publicKeys);
            c0 = result[0];
            s = result[1];
            keyImage = result[2];
            /*
            ...
            const c1 = '0x16f154c8b054472b27fa5ddfdc6efaef113f287567f0bdfe58a8890d8c6fc4ec'
